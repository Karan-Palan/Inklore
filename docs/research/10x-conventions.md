---
title: 10x-mono conventions (research for Inkflow)
owner: Inkflow Engineering
...
- **ElevenLabs / fal**: not present anywhere in the repo (grep for `elevenlabs`, `fal_client`, `fal.ai` returned nothing). Inkflow will need to add these following the `email_sender.py` shape.
...
## 7. How a new app (Inkflow) should be laid out to match 10x
...
inkflow/
  ...
  ios/
    ...
    App/InkflowApp.xcodeproj, App/InkflowApp/InkflowAppApp.swift
    Sources/InkflowKit/ {API/{APIClient,Models,Stores}.swift, Theme/{Theme.swift,Generated/DesignTokens.swift},
                          Primitives/, Glass/GlassChrome.swift, Mocks/{MockTransport,MockHandlers,Fixtures}.swift,
                          Resources/Fixtures/*.json, Gallery/, Views/}
    Tests/InkflowKitTests/ {FixtureParityTests, MockTransportTests, ScreenSnapshotTests, __Snapshots__/}
    scripts/export-fixtures.mjs
    docs/ {README, architecture, local-development, testing, design-system, navigation, deployment}
  web/
...
**Copy verbatim (rename `10x`/`TenX` -> `Inkflow`, drop 10x-only domains):**
...
- `packages/design-tokens/` in full (`package.json`, `schema.json`, `scripts/generate.mjs`, `tests/`, `docs/token-model.md`); then edit `tokens.json` values for Inkflow's palette, keep the semantic names.
...
**Adapt:**
- Root `AGENTS.md`: keep structure and every rule list; replace mission/domains, drop fleet/preview/admin-stats/prompt-corpus sections, replace the developer allowlist with Inkflow's handles (identity check via `gh api user --jq .login` stays), shorten the repository map to the projects Inkflow has.
...
- `docs/architecture.md`, `deployment-*.md`, `domains-and-environments.md`, `observability.md`, `security-and-privacy.md`: keep headings and invariants list; fill Inkflow services.
- Feature packets: create per Inkflow journey (library, reader, sync, audio/narration, onboarding, paywall, account) from `_template/`; add `registry.yml`.
...
- iOS navigation: keep the "app tier = native `TabView`, deep task tier = full-screen cover with glass pill" model but define Inkflow's tabs; keep `OPEN_*` debug deep links.
...
- Analytics registry: start `events.json` empty and register every Inkflow event before emitting; add an iOS generated consumer (10x planned but never built one).
- Add what 10x lacks and Inkflow needs: iOS PostHog/Sentry setup, iOS Keychain-backed Supabase session, an iOS release pipeline doc (`ios/docs/deployment.md` in 10x is still a draft), a generated Swift client only if the team decides to close the gap 10x left open.
