## ReadSync — App Clone Brief (Kindle-inspired)
...
- Provider readiness is external: Supabase Auth must enable Apple + Google with credentials, list redirect `readsync://auth-callback`, and (for frictionless email) disable Confirm Email. Apple requires the Sign in with Apple entitlement on the bundle ID.
