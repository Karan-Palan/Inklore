# ReadSync - Product and Build Plan
...
Decision to confirm: **stay on the 10x-managed stack declared in `tenx.yaml`** (Neon Postgres + Better Auth + R2 + FastAPI on the 10x Paid Backend runtime) rather than re-platforming to Supabase/Vercel. Reasons: auth, DB, storage, jobs and client facades (`ios/Generated/*`) are already generated and wired; 10x's own monorepo conventions (router -> service -> DB helper, `NNN_name.sql` migrations with owner-scoped RLS, leased poller jobs, provider wrappers as one httpx module each) transfer 1:1. Vercel is used by 10x only for its own web app; a Readsync web app can follow that later.
...
- Salvage from `/tmp/readsync-scaffold`: `FalClient`, `ElevenLabsClient` (sentence folding), `ColdOpenView`, `ReadAloudPlayer` - port into the 10x layering (backend owns fal/ElevenLabs keys; iOS never calls providers directly).
