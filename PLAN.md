...
The existing app makes this worth pursuing because Readsync is not starting from a slide deck. The repository already has a native reader, file import, book search, read-along narration, highlights, notes, stats, authentication, storage declarations, database migrations, and an email backend.

### What should the hackathon product be?

**Readsync Alive: drop in any book and hear it performed by a cast while every spoken word stays synchronized with the page. Interrupt at any moment to ask a question and Readsync saves the answer as a cited, rendered note.**
...
The complete Readsync company remains the universal reading and memory system around that wedge:
...
Those results favor one obvious before-and-after moment on a real phone. Readsync Alive provides that moment while reusing the strongest parts of the current app.
...
```text
origin https://github.com/Karan-Palan/readsync-76b146
```
...
| Capability | Speechify / ElevenReader | Readwise Reader | Kindle / Audible | Readsync launch | Readsync Alive |
|---|---|---|---|---|---|
...
The downloaded Speechify reference sequence contains roughly 45 relevant screens. It demonstrates useful patterns, but Readsync should not copy its length.
...
- Borrowable on the source service: Open in source service, not copied into Readsync.
...
- The app target is thin. Reusable code belongs in a `ReadsyncKit` Swift package.
...
4. Create a thin iOS app target plus local `ReadsyncKit` package.
...
- Private reading content is not used to train Readsync models.
...
- Keep Readsync. Do not switch to an unrelated hackathon idea.
- Reposition the hackathon build around Readsync Alive.
